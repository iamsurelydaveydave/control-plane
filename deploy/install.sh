...
configure_dns() {
    ...
    # Test if DNS resolution works by resolving a known domain
    if host google.com >/dev/null 2>&1 || nslookup google.com >/dev/null 2>&1 || getent hosts google.com >/dev/null 2>&1; then
        log_success "DNS resolution working"
        return 0
    fi
    
    log_warn "DNS resolution not working — configuring fallback DNS servers"
    
    # Check if systemd-resolved is in use
    if systemctl is-active systemd-resolved >/dev/null 2>&1; then
        log "Configuring systemd-resolved with public DNS servers..."
...
EOF
        ...
        sleep 2
        if host google.com >/dev/null 2>&1 || nslookup google.com >/dev/null 2>&1 || getent hosts google.com >/dev/null 2>&1; then
            log_success "DNS configured successfully (Google + Cloudflare DNS)"
        else
            log_warn "DNS may still have issues — continuing anyway"
        fi
    else
        # Fallback: directly write to /etc/resolv.conf
        log "systemd-resolved not active — writing to /etc/resolv.conf directly..."
...
# Generated by Control Plane installer
nameserver 8.8.8.8
nameserver 8.8.4.4
nameserver 1.1.1.1
EOF
