        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p class="text-sm mt-1 capitalize">
              ...
              <span
                v-if="app.source?.type === 'image' || !app.source"
                class="text-muted"
              > · {{ app.source?.image ?? app.image }}</span>
              <span
                v-else
                class="text-muted"
              > · {{ app.source?.gitUrl ?? app.source?.repository }}</span>
            </p>
          </div>
          <div v-if="app.proxy">
            <label class="text-xs text-muted uppercase tracking-wide">Port</label>
            <p class="font-mono text-sm mt-1">
              {{ app.proxy.appPort ?? app.proxy.port }}
            </p>
          </div>
          <div>
      ...
      <div class="rounded-xl border border-default bg-elevated/50 p-6">
        ...
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <span class="text-muted">replica{{ (app.desiredReplicas ?? 1) > 1 ? 's' : '' }}</span>
          </div>
          <UBadge
            v-if="(app.desiredReplicas ?? 1) === 0"
            label="Stopped"
            color="neutral"
            variant="subtle"
          />
        ...
        <p class="text-sm text-muted mt-3">
          App runs as a K8s Deployment. Scale replicas for high availability.
        </p>
      </div>
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              <p class="text-muted">
                ...
                with {{ app?.desiredReplicas ?? 1 }} replica{{ (app?.desiredReplicas ?? 1) > 1 ? 's' : '' }}?
              </p>
